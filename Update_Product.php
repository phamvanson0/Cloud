...
if (isset($_GET["id"])) {
	...
	$sqlstring = "Select Product_Name, Price, SmallDesc, DetailDesc, ProDate, Pro_qty,
		Pro_image,Cat_ID,StaName from product where Product_ID='$id'";
	...
	<div class="coverblock">
		...
		<form id="frmProduct" name="frmProduct" method="post" enctype="multipart/form-data" action="" class="form-horizontal" role="form">
			...
			<div class="form-group">
				...
				<div class="col-sm-10">
					<input type="text" name="txtStaName" id="txtStaName" class="form-control" 
					placeholder="Performance Staff" value='<?php echo $staff;?>'/>
				</div>
			</div>
...
if (isset($_POST["btnUpdate"])) {
	...
	if (trim($staff) == "") {
		$err .= "<li>Enter performance staff, please</li>";
	}
	if ($err != "") {
		echo "<ul>$err</ul>";
	} else {
		if ($pic['name'] != "") {
			if (
				...
				if ($pic['size'] <= 614400) {
					...
					if (mysqli_num_rows($result) == 0) {
						...
						Pro_image='$filePic',Cat_ID='$category',StaName='$staff',
			...
			if (mysqli_num_rows($result) == 0) {
				...
				Pro_qty=$qty, Cat_ID='$category', StaName='$staff', 
				ProDate='" . date('Y-m-d H:i:s') . "' WHERE Product_ID='$id'";
